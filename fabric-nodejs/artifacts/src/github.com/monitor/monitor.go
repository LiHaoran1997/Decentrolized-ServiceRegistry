...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
    ...
    } else if function == "Monitor_InstanceByOne" {
        return t.Monitor_InstanceByOne(stub, args)
    } else {
...
func (t *SimpleChaincode)Monitor_InstanceByOne(stub shim.ChaincodeStubInterface, args []string) pb.Response{
    if len(args) !=3 {
        return shim.Error("Incorrect arguments. Expecting a key and a value")
    }
    serviceName := args[0]
    instanceIp:=args[1]
    monitorIp:=args[2]
    bstatus, err := stub.GetState("springcloud.monitor."+serviceName+"."+instanceIp+"."+"@@"+monitorIp)
    if err != nil||bstatus==nil {
        return shim.Error("Query form status fail, form number:" +"springcloud.monitor."+serviceName+"."+instanceIp+"@@"+monitorIp)
    }
